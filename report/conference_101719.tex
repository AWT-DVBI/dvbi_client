...
To retrieve scheduleinfo meta data we used the ScheduleInfoEndpoint descirebed in [Quelle: dvbi standard] and shown in figure \ref{fig:scheduleInfoEndpoint}.
...
The naive implementation had a runtime of O(n2) because each ScheduleEvent field required the corresponding program field marked by CRID.

By using a HashMap, we could reduce the time from 10 minutes to 30 seconds when retrieving the metadata of each service element at once
